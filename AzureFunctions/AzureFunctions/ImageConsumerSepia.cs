...
namespace AzureFunctions
{
    ...
    {
        const string ImagesToConvertRoute = ConfigSettings.SEPIAIMAGES_CONTAINERNAME + "/{name}";

        /// <summary>
        /// Converts images uploaded into the imagestoconverttoSepia into gray scale format
        ...
        /// imagestoconverttoSepia continer
        /// 
        ...
        public static async Task Run([BlobTrigger(ImagesToConvertRoute, Connection = ConfigSettings.STORAGE_CONNECTIONSTRING_NAME)] CloudBlockBlob cloudBlockBlob, string name, ILogger log)
        {
            ...
            using (Stream blobStream = await cloudBlockBlob.OpenReadAsync())
            {
                ...
                ImageMode imageMode = ImageMode.Sepia;
